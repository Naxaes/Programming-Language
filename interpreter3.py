...
class Tokenizer:
    ...
    def skip_whitespace(self):
        while self.current_character.isspace() and self.current_character != '\n':
            self.current_character = self.stream.next()
    ...
    def indention(self, count=0):
        self.advance()  # Advance passed '\n'
        if self.current_character == '\n':
            count = self.indention()
        if self.current_character == ' ':
            count = self.indention(count + 1)
        if self.current_character == '\t':
            count = self.indention(count + 4)
        return count
    ...
    def get_next_token(self):
        ...
        while self.current_character == '/' and self.stream.peek() == '/':
            self.skip_comment()

        if self.current_character == '\n':
            level = self.indention() // 4
            self.next_token = Token(INDENTION, level)
            return Token(END_STATEMENT, END_STATEMENT)
...
class Parser:
    ...
    def program(self):
        ...
        return BlockNode(blocks)  # ProgramNode?

    def block(self):
        """
        block   :  (INDENTION statements)*

        return  :  BlockNode.
        """
        statements = []
        while self.current_indention == self.current_token.value:
            self.consume(INDENTION)
    ...
    def statement(self):
        ...
        if self.current_token.type == ID:  # Will fail when using user-defined functions.
            ...
            self.consume(END_STATEMENT)
            return node
        elif self.current_token.type == INDENTION:
            if self.current_token.value > self.current_indention:
                self.current_indention += 1
                return self.block()
            elif self.current_token.value < self.current_indention:
                self.current_indention -= 1
                self.consume(INDENTION)
            ...
            self.consume(THEN)
            self.consume(END_STATEMENT)
            return WhileLoopNode(condition, self.statement())
        elif self.current_token.type == END_STATEMENT:
            self.consume(END_STATEMENT)
            return NoOperationNode()
