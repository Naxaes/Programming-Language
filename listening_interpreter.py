...
KEYWORDS = {
    ...
    'at'     : Token(DECLARE_ASSIGN, DECLARE_ASSIGN),   # Is always confused with 'set'.
    ...
    'break'   : Token(END_STATEMENT, END_STATEMENT),
    'variable': Token(ID, 'variable'),
    # '\t'    : Token(INLINE, INLINE),
    # '    '  : Token(INLINE, INLINE),
    'set'    : Token(DECLARE_ASSIGN, DECLARE_ASSIGN),
    ...
    # FIRST                                               SECOND
    '*'      : defaultdict(lambda: Token(MUL, MUL), **{'*': Token(POW, POW)}),
    '='      : defaultdict(lambda: Token(ASSIGN, ASSIGN), **{'=': Token(EQUAL, EQUAL)}),
    '+'      : defaultdict(lambda: Token(ADD, ADD), **{'+': Token(INCREMENT, INCREMENT)}),
    '-'      : defaultdict(lambda: Token(SUB, SUB), **{'-': Token(DECREMENT, DECREMENT)}),
    ':'      : defaultdict(lambda: Token(DECLARE, DECLARE), **{'=': Token(DECLARE_ASSIGN, DECLARE_ASSIGN)}),
    '>'      : defaultdict(lambda: Token(GREATER_THAN, GREATER_THAN),
                           **{'=': Token(GREATER_EQUAL_THAN, GREATER_EQUAL_THAN)}),
    '<'      : defaultdict(lambda: Token(LESS_THAN, LESS_THAN), **{'=': Token(LESS_EQUAL_THAN, LESS_EQUAL_THAN)}),
    '!'      : defaultdict(lambda: None, **{'=': Token(NOT_EQUAL, NOT_EQUAL)}),
...
class Tokenizer:
    ...
    def read_identifier(self):
        ...
        if text is None:
            best_match, best_matching_value = None, 0.2  # Need a minimum matching value of 0.2
            for keyword in KEYWORDS:
                matching_value = SequenceMatcher(None, result, keyword).ratio()
                if matching_value > best_matching_value:
                    best_matching_value = matching_value
                    best_match = keyword
            token = KEYWORDS.get(best_match)
            if token:
                print('Changing {} to {} with a matching value of {}.'.format(result, best_match, best_matching_value))
                return token
            else:
                raise Exception('Could not find a close match to {}'.format(result))
        else:
            return text
    ...
    def condition(self, previous_condition=None):
        ...
        EQUAL, NOT_EQUAL, GREATER_THAN, GREATER_EQUAL_THAN, LESS_THAN, LESS_EQUAL_THAN, NOT, IS) \
            ...
            if isinstance(node.left, NameNode):
                ...
                if isinstance(node.right, NameNode):
                    raise Exception('Only left variable can be chained. Switch {} with {}.'.format(node.left.value,
                                                                                                   node.right.name))
        else:
...
class Interpreter:
    ...
    def visit_ReAssign(self, node):
        ...
        if atom.type != new_atom.type:
            raise TypeError('{} is not compatible with {}'.format(atom, new_atom))
        else:
            namespace = self.current_namespace
...
with sr.Microphone() as source:
    ...
    while code is not None:
        ...
        try:
            ...
            print('Your code:\n\n\t', '\n\t'.join(code.split('break')))

            interpreter = Interpreter(Parser(Tokenizer(InputStream(code))))
